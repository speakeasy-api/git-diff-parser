...
func ApplyFile(pristine, patchData []byte) ([]byte, error) {
	...
	for _, hunk := range hunks {
		...
		if !matched {
			...
			outLines = appendConflict(outLines, sourceLines[conflictStart:conflictEnd], desiredLines(hunk))
			cursor = conflictEnd
			continue
		}
...
func appendConflict(out []fileLine, ours, theirs []fileLine) []fileLine {
	out = append(out, fileLine{text: "<<<<<<< Current (Your changes)", hasNewline: true})
	...
	out = ensureTrailingNewline(out)
	out = append(out, fileLine{text: ">>>>>>> New (Generated by Speakeasy)", hasNewline: true})
	return out
}
